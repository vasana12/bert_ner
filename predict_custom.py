...
def read_input_file(pred_config):
    ...
    sql = "select * from htdocs where keyword=\'%s\' and channel=\'%s\' and publishtime>=\'%s\' and publishtime<=\'%s\' and postid=%s limit %s"\
          %(keyword,channel,stdate, endate,3018104,nurl)
    print(sql)
    curs.execute(sql)
    rows = curs.fetchall()
    text = ''
    ...
    for row in rows:
        text += row['htmltext']
    text.replace("[^ㄱ-ㅎㅏ-ㅣ가-힣]","")
    text = re.sub(u"\\d+", " ", text)
    text = re.sub(u"\\s+", " ", text)
    text.replace(keyword,' '+keyword)
    text.replace("  "," ")
    words = text.split()
    lines.append(words)
...
def wordCount(loc,att):
    counter_list = []
    counter = Counter(loc)
    c= counter.most_common()
    print('count=',c)
    for index,i in enumerate(c):
        count_dict = {'loc':i[0], 'count':i[1], 'att':att[index]}
        counter_list.append(count_dict)
    print(counter_list)
    table = pd.DataFrame(counter_list,columns=('loc','count','att'))
    table.to_excel('loc_count/'+'수박'+'.xlsx',encoding='utf-8',index=True)
        ...
        for words, preds in zip(lines, preds_list):
            ...
            for word, pred in zip(words, preds):
                ...
                if pred == 'O':
                    ...
                    if pred=='LOC-B' or pred=='LOC-I' or pred=='ORG-I' or pred=='ORG-B':
                        att.append(pred)
                        loc.append(word)
                        print('word,pred:',word,pred)
